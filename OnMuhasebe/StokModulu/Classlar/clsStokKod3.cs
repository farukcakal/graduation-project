...
namespace OnMuhasebe.StokModulu.Classlar
{
    class clsStokKod3
    {
        ...
        {
            string sql = "SELECT * FROM TBLSTOKKOD3 ORDER BY KOD";
            DataTable tablo = DBase.TabloCek(sql);
            return tablo;
        }
        public void Ekle(string Kod, string Aciklama, string UserId)
        {
            string sql = "EXECUTE SP_STOKKOD3_EKLE '" + Kod + "','" + Aciklama + "'," + UserId;
            DBase.Isle(sql);
        }
        public void Sil(string Id)
        {
            string sql = "EXECUTE SP_STOKKOD3_SIL " + Id;
            DBase.Isle(sql);
        }
        public void Guncelle(string Id, string Kod, string Aciklama, string UserId)
        {
            string sql = "EXECUTE SP_STOKKOD3_GUNCELLE '"+ Id + "','" + Kod + "','" + Aciklama + "'," + UserId;
            DBase.Isle(sql);
        }
        public DataRow Ac (string Id)
        {
            return DBase.Satir("SELECT * FROM TBLSTOKKOD3 WHERE ID = " + Id);
        }
        ...
        public String IdAl(string Kod, Boolean YoksaAcilsinMi = false, string UserId = "-1")
        {
            try
            {
                return DBase.SatirCek("SELECT * FROM TBLSTOKKOD3 WHERE KOD ='" + Kod + "'")["ID"].ToString();
            }
            catch (Exception)
            {
